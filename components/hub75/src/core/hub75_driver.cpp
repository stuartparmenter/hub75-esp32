...
bool Hub75Driver::begin() {
  ...
  // Setup DMA (platform-specific setup happens inside)
  if (!dma_->setup_dma(nullptr, nullptr, 0)) {
    ESP_LOGE(TAG, "Failed to setup DMA");
    dma_->shutdown();
    delete dma_;
    dma_ = nullptr;
    return false;
...
void Hub75Driver::clear_buffer() {
  if (!config_.double_buffer) {
    ESP_LOGW(TAG, "clear_buffer() called but double buffering not enabled");
    return;
  }
...
void Hub75Driver::set_basis_brightness(uint8_t brightness) {
  if (dma_) {
    dma_->set_basis_brightness(brightness);
  }
}
...
float Hub75Driver::get_refresh_rate() const {
