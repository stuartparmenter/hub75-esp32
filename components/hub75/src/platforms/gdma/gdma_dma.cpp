...
bool GdmaDma::setup_dma(uint8_t *buffer_a, uint8_t *buffer_b, size_t size) {
...
bool GdmaDma::allocate_row_buffers() {
  size_t pixels_per_bitplane = virtual_width_;  // Fixed size per bit plane (no padding)
  ...
  for (uint16_t dy = 0; dy < h; dy++) {
    for (uint16_t dx = 0; dx < w; dx++) {
      ...
      for (int bit = 0; bit < bit_depth_; bit++) {
        uint16_t *buf = (uint16_t *) (row_buffers_[row].data + (bit * virtual_width_ * 2));
...
void GdmaDma::clear() {
  ...
  for (int row = 0; row < num_rows_; row++) {
    for (int bit = 0; bit < bit_depth_; bit++) {
      ...
      for (uint16_t x = 0; x < virtual_width_; x++) {
...
bool GdmaDma::validate_brightness_config() {
  ...
  // Edge Case 1: Latch blanking must be less than virtual width
  // Prevents underflow in: max_pixels = (virtual_width_ - latch_blanking) >> rightshift
  if (latch_blanking >= virtual_width_) {
    ESP_LOGE(TAG, "Invalid config: latch_blanking (%u) >= virtual_width (%u)", latch_blanking, virtual_width_);
    return false;
  }

  // Edge Case 2: Virtual width must be large enough for reasonable operation
  // Need space for: data pixels + latch blanking + safety margins
  if (virtual_width_ < 8) {
    ESP_LOGE(TAG, "Invalid config: virtual_width (%u) too small (minimum 8)", virtual_width_);
    return false;
  }
  ...
  for (int bit = 0; bit < bit_depth_; bit++) {
    ...
    const int max_pixels = (virtual_width_ - latch_blanking) >> rightshift;

    if (max_pixels < 2) {
      ESP_LOGE(TAG,
               "Invalid config: bit %d has max_pixels=%d (need >=2). "
               "Increase virtual_width or decrease latch_blanking or adjust bit_depth.",
               bit, max_pixels);
      ESP_LOGE(TAG, "  Config: virtual_width=%u, latch_blanking=%u, bit_depth=%u, lsbMsbTransitionBit=%u",
               virtual_width_, latch_blanking, bit_depth_, lsbMsbTransitionBit_);
      return false;
    }
  }

  ESP_LOGI(TAG,
           "Brightness configuration validated: virtual_width=%u, latch_blanking=%u, all bit planes have sufficient "
           "headroom",
           virtual_width_, latch_blanking);
  return true;
}
  ...
  for (int row = 0; row < num_rows_; row++) {
    ...
    for (int bit = 0; bit < bit_depth_; bit++) {
      ...
      if (bit == 0) {
        ...
        if (row == 0) {
          addr_for_buffer = (num_rows_ - 1) & ADDR_MASK;  // Wrap to last row
        } else {
          addr_for_buffer = (row - 1) & ADDR_MASK;
        }
      ...
      buf[virtual_width_ - 1] |= (1 << LAT_BIT);
    }
  }
...
void GdmaDma::set_brightness_oe() {
  ...
  for (int row = 0; row < num_rows_; row++) {
    for (int bit = 0; bit < bit_depth_; bit++) {
      ...
      const int x_min = (virtual_width_ - display_pixels) / 2;
      const int x_max = (virtual_width_ + display_pixels) / 2;

      // Debug validation: verify buffer bounds
      assert(x_min >= 0 && "x_min underflow");
      assert(x_max <= virtual_width_ && "x_max exceeds buffer bounds");
      ...
      const int last_pixel = virtual_width_ - 1;
      ...
      for (int i = 0; i < latch_blanking && i < virtual_width_; i++) {
        buf[i] |= (1 << OE_BIT);
      }
    }
  }
...
bool GdmaDma::build_descriptor_chain() {
  size_t pixels_per_bitplane = virtual_width_;          // Fixed size per bit plane
  ...
  for (int row = 0; row < num_rows_; row++) {
    for (int bit = 0; bit < bit_depth_; bit++) {
      uint8_t *const bit_buffer = row_buffers_[row].data + (bit * bytes_per_bitplane);
...
void GdmaDma::calculate_bcm_timings() {
  ...
  // Buffer contains virtual_width_ pixels with LAT on last pixel
  // Latch blanking is handled via OE bits, not extra pixels
  const uint16_t buffer_pixels = virtual_width_;  // LAT is on last pixel, not extra
