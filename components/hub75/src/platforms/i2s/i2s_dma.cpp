...
bool I2sDma::setup_dma(uint8_t *buffer_a, uint8_t *buffer_b, size_t size) {
...
void I2sDma::calculate_bcm_timings() {
  ...
  const uint16_t buffer_pixels = virtual_width_;
...
bool I2sDma::build_descriptor_chain() {
  size_t pixels_per_bitplane = virtual_width_;
  ...
  for (int row = 0; row < num_rows_; row++) {
    for (int bit = 0; bit < bit_depth_; bit++) {
      ...
      for (int rep = 0; rep < repetitions; rep++) {
        lldesc_t *const desc = &descriptors_[desc_idx];
